...
;********************************************************************************************************
    ...
    EXPORT  OSCtxSw
    EXPORT  OSIntCtxSw
    EXPORT  PendSV_Handler	;alter by jiang for stm32 for startu
...
;********************************************************************************************************
        ...
        AREA |.text|, CODE,READONLY, ALIGN=2;AREA |.text| 表示：选择段|.text|。
					    ...
					    ;ALIGN=2 表示4 字节对齐。若ALIGN=n，这2^n 对
	THUMB				    ;Thumb代码
...
OSCtxSw
    ; 悬起(允许)PendSV 中断(看不懂这段代码的，可参考前面见过的OSStartHighRdy )
    LDR     R0, =NVIC_INT_CTRL                                  ; Trigger the PendSV exception (causes context switch)
    LDR     R1, =NVIC_PENDSVSET		; 中断控制及状态寄存器ICSR 的位28
    STR     R1, [R0]			; 设置中断控制及状态寄存器ICSR 位28 为1
    					;以悬起(允许)PendSV 中断
    BX      LR
...
PendSV_Handler			;alter by jiang for stm32
